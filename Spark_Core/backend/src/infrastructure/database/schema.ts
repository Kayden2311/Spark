...
export const platformRoleAssignments = spark.table("platform_role_assignments", { ...mutable(), userId: ref("user_id").notNull(), role: state("role", ["campaign_moderator"]).notNull(), grantedByUserId: ref("granted_by_user_id").notNull(), revokedAt: instant("revoked_at") }).enableRLS();
