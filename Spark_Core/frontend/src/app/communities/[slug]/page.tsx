...
export default async function CommunityDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="communityDetail">
    <header className="communityHeader"><Link className="communityBrand" href="/">Spark</Link><nav aria-label="Main navigation"><Link href="/">Home</Link><Link href="/communities">Communities</Link><Link href="/pricing">Pricing</Link><Link href="/workspace">Workspace</Link></nav></header>
